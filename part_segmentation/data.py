...
class PartNormalDataset(Dataset):
    def __init__(self, npoints=2500, split='train', with_normal=True, with_one_hot_shape_id=True,
    ...
    def __getitem__(self, index):
        if index in self.cache:
            coords, normal, seg, cls = self.cache[index]
        else:
            fn = self.datapath[index]
            ...
            if len(self.cache) < self.cache_size:
                self.cache[index] = (coords, normal, seg, cls)
        ...
        return point_set, cls, seg[choice].transpose()
