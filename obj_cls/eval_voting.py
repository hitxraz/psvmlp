...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/pvmlp_test.yaml', help='config file')
...
def _init_():
    ...
    if args.arch == 'pvmlp':
        os.system('cp model/pvmlp.py checkpoints' + '/' + args.exp_name + '/' + 'pvmlp_vote.py.backup')
...
def test(args, io):
    ...
    #     model = PAConv(args).to(device)
    if args.arch == 'pvmlp':
        from model.pvmlp import PVMLP  
        model = PVMLP().to(device)
    else:
